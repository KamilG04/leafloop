...
namespace LeafLoop.Services
{
    public class TransactionService : ITransactionService
    {
        ...
        // POPRAWIONA METODA GetTransactionByIdAsync
        public async Task<TransactionDto> GetTransactionByIdAsync(int id)
        {
            _logger.LogInformation("Getting transaction by ID: {TransactionId}", id);
            try
            {
                // Użyj dedykowanej metody repozytorium, jeśli istnieje, np.:
                // var transaction = await _unitOfWork.Transactions.GetTransactionWithBasicDetailsAsync(id);
                // LUB pobierz standardowo i załaduj ręcznie (mniej wydajne) lub polegaj na mapowaniu
                ...
                // >>> USUNIĘTO: await _unitOfWork.Transactions.LoadRelatedEntitiesAsync(transaction); <<<

                // TODO: Upewnij się, że AutoMapper lub zapytanie ładuje dane dla SellerName, BuyerName etc.
                // Najlepiej stworzyć w ITransactionRepository metodę np. GetTransactionForDtoAsync(id),
                // która użyje Include(t => t.Seller).Include(t => t.Buyer).Include(t => t.Item).ThenInclude(i => i.Photos)
        ...
        public async Task<TransactionWithDetailsDto> GetTransactionWithDetailsAsync(int id) { /* ... jak poprzednio ... */
             try
            {
                var transaction = await _unitOfWork.Transactions.GetTransactionWithDetailsAsync(id); // Ta metoda repo MUSI ładować wszystkie potrzebne dane
        ...
        public async Task<IEnumerable<TransactionDto>> GetTransactionsByUserAsync(int userId, bool asSeller = false) { /* ... jak poprzednio ... */
            try
            {
        ...
        public async Task<IEnumerable<TransactionDto>> GetTransactionsByItemAsync(int itemId) { /* ... jak poprzednio ... */
            try
            {
                // Zakładamy, że repozytorium zwraca transakcje z załadowanymi danymi potrzebnymi dla TransactionDto
        ...
        public async Task<IEnumerable<TransactionDto>> GetTransactionsByStatusAsync(TransactionStatus status) { /* ... jak poprzednio ... */
             try
            {
                 // Zakładamy, że repozytorium zwraca transakcje z załadowanymi danymi potrzebnymi dla TransactionDto
        ...
        public async Task<int> InitiateTransactionAsync(TransactionCreateDto transactionDto, int buyerUserId) // Poprawiona sygnatura
        {
            ...
            {
                // 1. Pobierz przedmiot
                var item = await _unitOfWork.Items.GetByIdAsync(transactionDto.ItemId); // Upewnij się, że ładuje Usera jeśli potrzebujesz SellerId od razu
                if (item == null) throw new KeyNotFoundException($"Item with ID {transactionDto.ItemId} not found.");
                if (!item.IsAvailable) throw new InvalidOperationException($"Item with ID {transactionDto.ItemId} is not available for transaction.");
                if (item.UserId == buyerUserId) throw new InvalidOperationException("You cannot initiate a transaction for your own item.");

                // 2. Sprawdź istniejące transakcje (PRZENIESIONE PRZED UTWORZENIEM NOWEJ)
                ...
                // 3. Utwórz transakcję (ręcznie, bez Offer, bez InitialMessage)
                var transaction = new Transaction
                {
                    ItemId = transactionDto.ItemId,
                    BuyerId = buyerUserId,
                    SellerId = item.UserId, // Zakładając, że item ma UserId właściciela
                    StartDate = DateTime.UtcNow,
                    LastUpdateDate = DateTime.UtcNow, // <<< ZAKŁADAM, ŻE DODAŁEŚ TO DO MODELU Transaction I DO BAZY DANYCH
                    Status = TransactionStatus.Pending, // Ustaw odpowiedni status początkowy
                    Type = transactionDto.Type
                };

                // 4. Dodaj i zapisz
                await _unitOfWork.Transactions.AddAsync(transaction);
                await _unitOfWork.CompleteAsync(); // Zapisz transakcję

                 _logger.LogInformation("Transaction initiated successfully. TransactionID: {TransactionId}", transaction.Id);

                // 5. !!! USUNIĘTO TWORZENIE WIADOMOŚCI !!!

                // 6. TODO: Wyślij powiadomienie do Sprzedającego

                return transaction.Id;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while initiating transaction for ItemId {ItemId} by UserID {BuyerId}", transactionDto.ItemId, buyerUserId);
                throw;
            }
        }
        ...
        public async Task UpdateTransactionStatusAsync(int id, TransactionStatus status, int userId)
        {
            ...
            try
            {
                ...
                transaction.LastUpdateDate = DateTime.UtcNow; // <<< ZAKŁADAM, ŻE DODAŁEŚ TO DO MODELU Transaction I DO BAZY DANYCH
            ...
            {
                _logger.LogError(ex, "Error occurred while updating transaction status: {TransactionId}, NewStatus: {NewStatus}, UserID: {UserId}", id, status, userId);
                throw;
            }
        }

        // Metoda CompleteTransactionAsync (bez zmian logiki)
        public async Task CompleteTransactionAsync(int id, int userId) { /* ... jak poprzednio ... */
            try
            {
                ...
                if (item != null && item.IsAvailable)
                {
                    ...
                    await _unitOfWork.CompleteAsync();
                     ...
                     _logger.LogWarning("Item {ItemId} not found during CompleteTransactionAsync for Transaction {TransactionId}.", transaction.ItemId, id);
                }
        ...
        public async Task CancelTransactionAsync(int id, int userId) { /* ... jak poprzednio ... */
             try
            {
        ...
        public async Task<bool> CanUserAccessTransactionAsync(int transactionId, int userId) { /* ... jak poprzednio ... */
             try
            {
        ...
        private bool IsValidStatusTransition(TransactionStatus currentStatus, TransactionStatus newStatus) { /* ... jak poprzednio ... */
            switch (currentStatus)
            {
                ...
                default:
                     _logger.LogWarning("Unknown current transaction status encountered in IsValidStatusTransition: {CurrentStatus}", currentStatus);
                    return false;
            }
         }

        private async Task UpdateUserEcoScoresAsync(int sellerId, int buyerId) { /* ... jak poprzednio ... */
             try
            {
                ...
                if (seller != null && buyer != null)
                {
                    ...
                    _logger.LogWarning("Could not find seller (ID: {SellerId}) or buyer (ID: {BuyerId}) to update EcoScores.", sellerId, buyerId);
                }
            }
            catch (Exception ex)
            {
