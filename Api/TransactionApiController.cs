...
public class TransactionsController : ControllerBase
{
    ...
    public async Task<IActionResult> InitiateTransaction([FromBody] TransactionCreateDto transactionDto)
    {
        ...
        try
        {
            ...
            {
                _logger.LogError("Transaction created (ID: {TransactionId}) but GetTransactionByIdAsync returned null.",
                    transactionId);
    ...
    public async Task<IActionResult> GetUserTransactions([FromQuery] bool asSeller = false)
    {
        ...
        {
            _logger.LogError(ex, "Error retrieving user transactions for UserID: {UserId}, asSeller: {AsSeller}",
                User.FindFirstValue(ClaimTypes.NameIdentifier), asSeller);
    ...
    {
        if (id <= 0) return this.ApiBadRequest("Invalid Transaction ID.");
        if (!Enum.IsDefined(typeof(TransactionStatus), statusUpdateDto.Status))
        ...
        try
        {
            ...
            var successMessage = $"Transaction status updated to {statusUpdateDto.Status}.";
            return this.ApiOk(successMessage);
        }
        ...
        {
            _logger.LogError(ex, "Error updating transaction status. TransactionId: {TransactionId}, UserID: {UserId}",
    ...
    public async Task<IActionResult> SendTransactionMessage(int id, [FromBody] TransactionMessageDto messageDto)
    {
        _logger.LogInformation(
            "API SendTransactionMessage START for TransactionID: {TransactionId}. Auth User: {AuthUser}", id,
        ...
        try
        {
            ...
            if (transaction.Status != TransactionStatus.Pending && transaction.Status != TransactionStatus.InProgress)
                return this.ApiBadRequest(
                    $"Cannot send messages for a transaction with status '{transaction.Status}'.");
            ...
            _logger.LogInformation(
                "Message sent via service. New MessageID: {MessageId} for TransactionID: {TransactionId}", messageId,
                id);
            ...
            {
                _logger.LogError(
                    "Message sent (ID: {MessageId}) but could not be retrieved for transaction {TransactionId}.",
                    messageId, id);
                return this.ApiOk("Message sent, but could not retrieve details.");
            }
        ...
        {
            _logger.LogError(ex,
                "Error sending transaction message. TransactionId: {TransactionId}, SenderId: {SenderId}", id,
    ...
    public async Task<IActionResult> RateTransaction(int id, [FromBody] TransactionRatingDto ratingDto)
    {
        ...
        try
        {
            ...
            {
                _logger.LogError(
                    "Rating submitted (ID: {RatingId}) but could not be retrieved for transaction {TransactionId}.",
                    ratingId, id);
